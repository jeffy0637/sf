...
class AbstractTrainer(object):
    ...
    訓練器類用於管理推薦系統模型的訓練和評估過程。
    AbstractTrainer是一個抽象類,其中fit()和evaluate()方法應根據不同的訓練和評估策略實現。
    """
...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    推薦系統中基礎訓練和評估策略的基礎訓練器。該類定義了大多數推薦系統模型的
    訓練和評估過程的常用功能,包括fit(),evaluate()和一些對模型訓練和評估有幫助的其他功能。
    
    通常,該類可為大多數推薦系統模型提供服務,如果模型的訓練過程是簡單地優化單個損失函數,
    不涉及任何複雜的訓練策略,例如對抗學習,預訓練等。
    
    初始化Trainer需要兩個參數:`config`和`model`。`config`記錄控制訓練和評估的參數信息,
    如`learning_rate`,`epochs`,`eval_step`等。更多信息可在[placeholder]中找到。
    `model`是Model類的實例化對象。
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None):
        ...
        單個epoch的訓練
        Args:
            train_data (DataLoader): 訓練數據
            epoch_idx (int): 当前epoch索引
            loss_func (function): 模型loss函数,默認為None。 如果為None,則loss函数將為
                :attr:`self.model.calculate_loss`。 默認為``None``。
        Returns:
            float/tuple: 該epoch中所有batch的loss之和。 如果每個batch的loss包含多個部分,而模型返回這些多個部分loss而不是loss總和,則會返回一個tuple,其中包含每個部分的loss總和。
        """
