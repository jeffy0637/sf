...
class TrainDataLoader(AbstractDataLoader):
    ...
    def _get_neg_sample(self):
        ...
        # 將互動權重轉為張量
        weight_tensor = torch.tensor(cur_data['weight'].values).type(torch.LongTensor).to(self.device)
        # 將互動權重張量拼接到batch_tensor中
        batch_tensor = torch.cat((batch_tensor, torch.unsqueeze(weight_tensor, 0)))
