...
class FREEDOM(GeneralRecommender):
    ...
    def get_norm_adj_mat(self):
        ...
        # 歸一化鄰接矩陣
        sumArr = (A > 0).sum(axis=1)
        diag = np.array(sumArr.flatten())[0] + 1e-7  # 避免除零
        diag = np.power(diag, -0.5)
        D = sp.diags(diag)
        L = D @ A @ D  # 使用 @ 符號表示矩陣乘法

        # 將歸一化的鄰接矩陣轉換為 PyTorch 稀疏張量
        L = sp.coo_matrix(L)
        row = L.row
        col = L.col
        i = torch.LongTensor(np.array([row, col]))
    ...
    def full_sort_predict_df(self, interaction):
        user = interaction[0]
