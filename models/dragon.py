...
class DRAGON(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        self.knn_k = config['knn_k'] # 物品语义图幾個邊
    ...
    def forward(self, interaction):
        ...
        # 检查 pos_scores 和 neg_scores 中是否包含无效值
        if torch.any(torch.isnan(pos_scores)) or torch.any(torch.isinf(pos_scores)):
            raise ValueError("pos_scores contains NaN or Inf values")
        if torch.any(torch.isnan(neg_scores)) or torch.any(torch.isinf(neg_scores)):
            raise ValueError("neg_scores contains NaN or Inf values")
    ...
    def calculate_loss(self, interaction):
        ...
        ## 整段為增加考慮weight
        interaction_weights = interaction[3]  # get the interaction weights
        # Apply the weights to the difference between the positive and negative scores
        if not torch.is_tensor(interaction_weights):
            interaction_weights = torch.Tensor(interaction_weights).to(self.device) # ensure interaction_weights is a tensor and send to device
        interaction_weights = interaction_weights.float()  # 将交互权重转换为浮点数
        #interaction_weights *= 0.1  # 缩放交互权重

        # 检查 interaction_weights 中是否包含无效值
        if torch.any(torch.isnan(interaction_weights)) or torch.any(torch.isinf(interaction_weights)):
            raise ValueError("interaction_weights contains NaN or Inf values")

        weighted_pos_scores = interaction_weights * pos_scores
        loss_value = -torch.mean(torch.log2(torch.sigmoid(weighted_pos_scores - neg_scores)))

        # v_preference和t_preference是從visual和textual信息中得到的用戶偏好表示
        reg_embedding_loss_v = (self.v_preference[user] ** 2).mean() if self.v_preference is not None else 0.0  # 計算視覺偏好表示的正則化損失
        reg_embedding_loss_t = (self.t_preference[user] ** 2).mean() if self.t_preference is not None else 0.0  # 計算視覺偏好表示的正則化損失

        reg_loss = self.reg_weight * (reg_embedding_loss_v + reg_embedding_loss_t) # 計算總的正則化損失

        # 如果模型是使用weighted_sum方式組合視覺和文本信息，則對這兩部分的權重也進行正則化
        if self.construction == 'weighted_sum':
            reg_loss += self.reg_weight * (self.weight_u ** 2).mean()
            reg_loss += self.reg_weight * (self.weight_i ** 2).mean()

        # 如果模型是使用concatenate方式組合視覺和文本信息，則對這部分的權重也進行正則化
        elif self.construction == 'cat':
            reg_loss += self.reg_weight * (self.weight_u ** 2).mean()

        # 如果模型是使用concatenate後通過MLP方式組合視覺和文本信息，則對MLP的權重也進行正則化
    ...
    def full_sort_predict_df(self, interaction):
        ...
        predict_list = score_matrix.tolist()
        res_df = pd.DataFrame({
